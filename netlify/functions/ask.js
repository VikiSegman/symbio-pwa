...
// ── Memory: embed + store (runs AFTER reply is returned) ──────────────────
  ...
  const req = https.request({
    ...
    res.on('end', () => {
      try {
        ...
        r2.write(sb); r2.end();
      } catch(e) { console.error('[ask] embed parse error:', e.message); }
    });
  });
...
- Language: respond in the SAME language the user used. Mixed He/En input → mixed He/En output.`;
    ...
    // Step 1: Call Claude (primary — must be fast)
    ...
    console.log('[ask] reply ok, length:', reply.length);

    // Step 2: Store memory in background (after reply — non-blocking)
